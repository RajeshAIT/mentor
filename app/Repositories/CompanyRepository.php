...
class CompanyRepository implements CompanyInterface
{
    ...
    public function getCompanyById($id)
    {
       ...
       if($companyCollection)
       {
          ...
          foreach($posts as $userPost)
          {
            ...
            $postDetails['thumbnail']=url('/api/post/media/'.$userPost->id);
            if($post_media_url->media_type_id!=1)
            {
                $postDetails['thumbnail']=url('/api/post/media/thumbnail/'.$userPost->id);
            }
           ...
           if ($userPost->post_type_id == '2')
            ...
            {
                $videoDetails['video_url']= url('/api/post/media/' . $media->post_id);
                $videoDetails['thumbnail']=url('/api/post/media/thumbnail/'.$media->post_id);
    ...
    public function invitepeople($request)
    {
        ...
        if($userEmail)
        {
            ...
            $link = "https://fiyuw.test-app.link?id=".$userEmail->id."&company_id=".$company_id->id."&email=".$request->email;
        } else
        {
            $user_id = 0;
            $link = "https://fiyuw.test-app.link?company_id=".$company_id->id."&email=".$request->email;
        }
